#!/usr/bin/env bash
# Quality control and IBS distance computation pipeline.
#
# Linta Mahboob, MIPT (MSc thesis, supervisor Prof. Oleg Balanovsky).
# Analysis run 2021; script tidied up for sharing in 2025.
#
# Converts per-cohort VCF files to PLINK binary format, merges cohorts,
# applies standard QC filters, and computes the pairwise IBS distance
# matrix used in the downstream analysis (script 02).
#
# Requires: SAMtools >= 1.12, BCFtools >= 1.12, PLINK v1.9
# Usage:    ./01_quality_control_pipeline.sh
# Input:    per-cohort VCF/BCF or PLINK binary files under ./raw_data
# Output:   final_dataset.bed/.bim/.fam, final_dataset.mdist(.id), QC log
...
echo "QC pipeline — South/West/Central Asian dataset"
echo "Started: $(date)"
